...
async def consumer_worker(name: str, queue: asyncio.Queue, dedup_store, stats, stop_event: asyncio.Event):
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if ok:
                stats["unique_processed"] = stats.get("unique_processed", 0) + 1
                logger.info("PROCESSED %s %s", event.topic, event.event_id)
            else:
                # tambahkan guard agar tidak KeyError dan langsung naikkan nilainya
                stats["duplicate_dropped"] = stats.get("duplicate_dropped", 0) + 1
